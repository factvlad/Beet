...
function makeFilmList(ar) {
    ...
    for (let index = 0; index < ar.length; index++) {
        list = list + `<li class="movie" data-isaddtofav="" data-name="${ar[index].Title}" id='${ar[index].imdbID}'> ${ar[index].Title} <button class='favourite' onclick='favourite(${index})'> Add To Favourite </button></li> \n <button onclick='details(${index})'>Details</button>`
    }
    list = list + "</ol>"
    res.append(list);
}
...
let movieFav = []
let list = $(".movie");
let favList = window.localStorage.getItem("ID");


$(document).ready(function () {
    let favList1 = window.localStorage.getItem("ID");
    if (favList1 != null) {
        let favAr = favList1.split(",");
        movieFav = favAr.slice();
    }
})
...
function favourite(i) {
    ...
    if (favList != null) {
        ...
        let eq = $.inArray(list[i].id, favList)
        //console.log(eq)
        if (eq == -1) {
            movieFav.push(list[i].id)
        } else {
            movieFav.splice(eq, 1);
        }
    ...
    //console.log(movieFav)
    window.localStorage.setItem("ID", movieFav.join());
